...
class CompositeAggsRowSet extends ResultRowSet<BucketExtractor> {
    ...
    CompositeAggsRowSet(List<BucketExtractor> exts, int[] columns, SearchResponse response, int limit, byte[] next, String... indices) {
        super(exts, columns);
        ...
        if (next == null) {
            ...
            if (next == null || size == 0 || remainingLimit == 0) {
                ...
                cursor = new CompositeAggregationCursor(next, exts, columns, remainingLimit, indices);
            }
        }
    }
