...
/** This is mostly a reuse of {@link org.apache.lucene.geo.Tessellator} but adapted for XY (non geo) Geometries
 *
 * @todo determine best way to reuse as much of  {@link org.apache.lucene.geo.Tessellator} as possible
 * (e.g., refactor private utility methods to be protected?)
 **/
final public class XYTessellator {
    ...
    public static final List<Triangle> tessellate(final XYPolygon polygon) {
        ...
        Node outerNode = createDoublyLinkedList(polygon.getPolyX(), polygon.getPolyY(), polygon.getWindingOrder(),
            0, GeoUtils.WindingOrder.CW);
    ...
    private static final Node createDoublyLinkedList(final double[] x, final double[] y, final GeoUtils.WindingOrder polyWindingOrder,
                                                     int startIndex, final GeoUtils.WindingOrder windingOrder) {
        Node lastNode = null;
        // Link points into the circular doubly-linked list in the specified winding order
        if (windingOrder == polyWindingOrder) {
            for (int i = 0; i < x.length; ++i) {
                lastNode = insertNode(x, y, startIndex++, i, lastNode);
            }
        } else {
    ...
    private static final Node eliminateHoles(final XYPolygon polygon, Node outerNode) {
        ...
        for(int i = 0; i < polygon.numHoles(); ++i) {
            ...
            Node list = createDoublyLinkedList(holes[i].getPolyX(), holes[i].getPolyY(), holes[i].getWindingOrder(),
                nodeIndex, GeoUtils.WindingOrder.CCW);
            if (list == list.next) {
                list.isSteiner = true;
            }
        ...
        return eliminateHoles(holeList, outerNode);
    }

    private static final Node eliminateHoles(List<Node> holeList, Node outerNode) {
    ...
    private static final boolean isEar(final Node ear, final boolean mortonOptimized) {
        ...
        while (node != ear.previous) {
            if (pointInEar(node.getX(), node.getY(), ear.previous.getX(), ear.previous.getY(),
                ear.getX(), ear.getY(), ear.next.getX(), ear.next.getY())
    ...
    private static final boolean mortonIsEar(final Node ear) {
        ...
        while (p != null && Long.compareUnsigned(p.morton, minZ) >= 0
            ...
            if (p.idx != ear.previous.idx && p.idx != ear.next.idx &&
                ...
                area(p.previous.getX(), p.previous.getY(),
                    p.getX(), p.getY(),
                    p.next.getX(), p.next.getY()) >= 0) return false;
            p = p.previousZ;
            ...
            if (n.idx != ear.previous.idx && n.idx != ear.next.idx &&
                pointInEar(n.getX(), n.getY(),
                    ...
                    ear.getX(), ear.getY(),
                    ear.next.getX(), ear.next.getY()) &&
                area(n.previous.getX(), n.previous.getY(),
                    n.getX(), n.getY(),
                    n.next.getX(), n.next.getY()) >= 0) return false;
            n = n.nextZ;
        }
    ...
    private static final Node insertNode(final double[] x, final double[] y, int index, int vertexIndex, final Node lastNode) {
        final Node node = new Node(x, y, index, vertexIndex);
        if(lastNode == null) {
            node.previous = node;
            node.previousZ = node;
            node.next = node;
    ...
    protected static class Node {
        ...
        protected Node(final double[] x, final double[] y, final int index, final int vertexIndex) {
            this.idx = index;
            this.vrtxIdx = vertexIndex;
            this.polyX = x;
            this.polyY = y;
            this.y = XYEncodingUtils.encode(polyY[vrtxIdx]);
            this.x = XYEncodingUtils.encode(polyX[vrtxIdx]);
            this.morton = BitUtil.interleave(this.x, this.y);
            this.previous = null;
            this.next = null;
            this.previousZ = null;
            this.nextZ = null;
