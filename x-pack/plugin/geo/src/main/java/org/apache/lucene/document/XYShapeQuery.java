...
/**
 * Base XYShape Query class providing common query logic for
 * {@link XYShapeBoundingBoxQuery}
 *
 * Note: this class implements the majority of the INTERSECTS, WITHIN, DISJOINT relation logic
 *
 * @lucene.experimental
 **/
abstract class XYShapeQuery extends Query {
  /** field name */
  final String field;
  /** query relation
  ...
  final XYShape.QueryRelation queryRelation;

  protected XYShapeQuery(String field, final XYShape.QueryRelation queryType) {
    if (field == null) {
  ...
  protected Relation relateRangeToQuery(byte[] minTriangle, byte[] maxTriangle, QueryRelation queryRelation) {
    ...
    if (queryRelation == XYShape.QueryRelation.DISJOINT) {
    ...
    return new ConstantScoreWeight(this, boost) {
      ...
      protected ScorerSupplier getIntersectScorerSupplier(LeafReader reader, PointValues values, Weight weight, ScoreMode scoreMode) throws IOException {
        ...
        return new RelationScorerSupplier(values, visitor, null, queryRelation) {
          ...
          public Scorer get(long leadCost) throws IOException {
            return getIntersectsScorer(XYShapeQuery.this, reader, weight, result, score(), scoreMode);
          }
        };
      }
      ...
      protected ScorerSupplier getScorerSupplier(LeafReader reader, PointValues values, Weight weight, ScoreMode scoreMode) throws IOException {
        ...
        return new RelationScorerSupplier(values, withinVisitor, disjointVisitor, queryRelation) {
          @Override
          public Scorer get(long leadCost) throws IOException {
            return getScorer(XYShapeQuery.this, weight, within, disjoint, score(), scoreMode);
          }
        };
      }
  ...
  protected boolean equalsTo(Object o) {
    return Objects.equals(field, ((XYShapeQuery)o).field) && this.queryRelation == ((XYShapeQuery)o).queryRelation;
  }
  ...
  private static abstract class RelationScorerSupplier extends ScorerSupplier {
    ...
    private IntersectVisitor getInverseIntersectVisitor(XYShapeQuery query, FixedBitSet result, int[] cost) {
      return new IntersectVisitor() {
    ...
    protected Scorer getIntersectsScorer(XYShapeQuery query, LeafReader reader, Weight weight,
    ...
    protected Scorer getScorer(XYShapeQuery query, Weight weight,
